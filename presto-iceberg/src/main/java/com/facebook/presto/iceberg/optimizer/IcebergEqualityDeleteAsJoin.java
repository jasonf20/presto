        ...
        public PlanNode visitTableScan(TableScanNode node, RewriteContext<Void> context)
        {
            ...
            for (Set<Integer> deleteSchema : deleteSchemas) {
                ...
                ImmutableMap<VariableReferenceExpression, ColumnHandle> deleteColumnAssignments = ImmutableMap.<VariableReferenceExpression, ColumnHandle>builder()
                        ...
                        .build();
                final List<VariableReferenceExpression> parentOutputs = parentJoin.getOutputVariables();
                Set<CanonicalJoinNode.EquiJoinClause> clauses = deleteColumnAssignments.entrySet().stream()
                        ...
                        .map(kvp -> {
                            VariableReferenceExpression left = parentOutputs.stream().filter(f -> ((IcebergColumnHandle) (kvp.getValue())).getName().startsWith(f.getName())).findFirst().get();
